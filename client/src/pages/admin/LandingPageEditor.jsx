...
const LandingPageEditor = () => {
  ...
  if (loading) {
    return (
      <MainLayout>
        <div className="flex items-center justify-center h-64 bg-bg-light min-h-screen">
          <div className="text-xl text-text-primary">טוען...</div>
        </div>
      </MainLayout>
    );
  }
  ...
  return (
    <MainLayout>
      <div className="p-4 sm:p-6 md:p-8 bg-bg-light min-h-screen">
        {/* Header */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 gap-4">
          <div>
            ...
            <p className="text-sm sm:text-base text-text-primary mt-2">עצב דף נחיתה מושך לשיווק האירועים והתורים שלך</p>
          </div>
          <div className="flex w-full sm:w-auto space-x-reverse space-x-3">
            <button
              onClick={() => navigate('/landing-pages')}
              className="flex-1 sm:flex-none bg-bg-light text-text-primary border border-accent-teal/20 px-4 sm:px-6 py-3 rounded-lg font-semibold transition-all hover:border-accent-teal/50 text-sm sm:text-base"
            >
              ביטול
            </button>
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex-1 sm:flex-none bg-gradient-to-r from-accent-teal to-accent-hover text-white px-4 sm:px-6 py-3 rounded-lg font-semibold disabled:opacity-50 transition-all shadow-lg shadow-accent-teal/50 hover:shadow-accent-teal/70 text-sm sm:text-base"
            >
              {saving ? '⏳ שומר...' : '💾 שמור'}
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-bg-light border border-accent-teal/30 rounded-xl shadow-lg p-4 sm:p-6 hover:border-accent-teal/50 hover:shadow-accent-teal/20 transition-all">
              <h3 className="text-lg sm:text-xl font-bold text-accent-teal mb-4">📝 פרטים בסיסיים</h3>
              
              <div className="space-y-4">
                <div>
                  <label className="block text-sm sm:text-base text-accent-teal font-medium mb-2">שם הדף</label>
                  <input
                    type="text"
                    value={formData.name}
                ...
                <div>
                  <label className="block text-sm sm:text-base text-accent-teal font-medium mb-2">סטטוס</label>
                  <select
                    value={formData.status}
                    onChange={(e) => setFormData({...formData, status: e.target.value})}
                    className="w-full px-3 sm:px-4 py-2 text-sm sm:text-base bg-bg-light border border-accent-teal/30 text-white rounded-lg focus:ring-2 focus:ring-accent-teal focus:border-transparent"
                  >
                ...
                <div>
                  <label className="block text-sm sm:text-base text-accent-teal font-medium mb-2">חבר לאירוע/תור</label>
                  <select
                    ...
                    className="w-full px-3 sm:px-4 py-2 text-sm sm:text-base bg-bg-light border border-accent-teal/30 text-white rounded-lg focus:ring-2 focus:ring-accent-teal focus:border-transparent mb-2"
                  >
            ...
            {/* Template Selection */}
            <div className="bg-bg-light border border-accent-teal/30 rounded-xl shadow-lg p-4 sm:p-6 hover:border-accent-teal/50 hover:shadow-accent-teal/20 transition-all">
              <h3 className="text-lg sm:text-xl font-bold text-accent-teal mb-4">🎨 בחר תבנית</h3>
                  ...
                  <button
                    ...
                    className={`p-3 sm:p-4 rounded-lg border-2 transition-all ${
                      formData.template === key
                        ...
                        : 'border-accent-teal/30 hover:border-accent-teal/50 bg-bg-light'
                    }`}
                  >
                    <div className="text-3xl sm:text-4xl mb-1 sm:mb-2">{template.preview}</div>
                    <div className="text-xs sm:text-sm font-medium text-text-primary">{template.name}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Hero Section */}
            <div className="bg-bg-light border border-accent-teal/30 rounded-xl shadow-lg p-4 sm:p-6 hover:border-accent-teal/50 hover:shadow-accent-teal/20 transition-all">
              <h3 className="text-lg sm:text-xl font-bold text-accent-teal mb-4">🎯 קטע גיבור (Hero)</h3>
              
              <div className="space-y-4">
                <div>
                  <label className="block text-sm sm:text-base text-accent-teal font-medium mb-2">כותרת ראשית</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm sm:text-base text-accent-teal font-medium mb-2">כותרת משנה</label>
                  <textarea
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm sm:text-base text-accent-teal font-medium mb-2">טקסט כפתור</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm sm:text-base text-accent-teal font-medium mb-2">צבע כפתור</label>
                    <input
                      type="color"
            ...
            <div className="bg-bg-light border border-accent-teal/30 rounded-xl shadow-lg p-4 sm:p-6 hover:border-accent-teal/50 hover:shadow-accent-teal/20 transition-all">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg sm:text-xl font-bold text-accent-teal">⭐ תכונות / יתרונות</h3>
                <button
                  onClick={addFeature}
                  className="bg-gradient-to-r from-accent-teal to-accent-hover text-white px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all shadow-lg shadow-accent-teal/50 hover:shadow-accent-teal/70"
                >
                  ➕ הוסף
                </button>
              </div>
              ...
              <div className="space-y-4">
                {formData.content.features.map((feature, index) => (
                  <div key={index} className="border border-accent-teal/30 bg-black/50 rounded-lg p-3 sm:p-4">
                    <div className="flex justify-between items-start mb-3">
                      <input
                        ...
                        placeholder="אייקון"
                        className="w-16 sm:w-20 px-2 py-1 bg-bg-light border border-accent-teal/30 text-white rounded text-xl sm:text-2xl text-center"
                      />
                      <button
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 text-sm sm:text-base bg-bg-light border border-accent-teal/30 text-white rounded-lg mb-2 focus:ring-2 focus:ring-accent-teal placeholder-gray-500"
                    />
                    <textarea
                      ...
                      rows="2"
                      className="w-full px-3 py-2 text-sm sm:text-base bg-bg-light border border-accent-teal/30 text-white rounded-lg focus:ring-2 focus:ring-accent-teal placeholder-gray-500"
                    />
                  </div>
                ))}
              </div>
            ...
            <div className="bg-bg-light border border-accent-teal/30 rounded-xl shadow-lg p-4 sm:p-6 hover:border-accent-teal/50 hover:shadow-accent-teal/20 transition-all">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-sm sm:text-base text-accent-teal font-medium mb-2">צבע משני</label>
                  <input
                    type="color"
                ...
                <div>
                  <label className="block text-sm sm:text-base text-accent-teal font-medium mb-2">צבע רקע</label>
                  <input
                    type="color"
          ...
          <div className="hidden lg:block lg:col-span-1">
            <div className="bg-bg-light border border-accent-teal/30 rounded-xl shadow-lg p-4 sm:p-6 sticky top-8">
              <h3 className="text-base sm:text-lg font-bold text-accent-teal mb-4">👁️ תצוגה מקדימה</h3>
